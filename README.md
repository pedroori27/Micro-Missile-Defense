...
6. O Arduino movimenta os servos motores (pan/tilt) para apontar na direção da pessoa detectada.
7. O vídeo é exibido com bounding box, centro da detecção, linha central da câmera e informações de ângulo/confiança sobrepostas.
...
- Bibliotecas Python:
  ...
  - `ultralytics` (YOLOv8)
  - `pyserial`
- [PlatformIO](https://platformio.org/) (extensão do VS Code ou CLI) para compilar e gravar o firmware do Arduino
...
3. O modelo `yolov8n.pt` é baixado automaticamente pela biblioteca `ultralytics` na primeira execução. Caso prefira baixar manualmente, ele está disponível nos releases oficiais do [Ultralytics](https://github.com/ultralytics/assets/releases) — coloque o arquivo na pasta do projeto.

4. Grave o firmware no Arduino usando o PlatformIO:
   ```bash
   cd firmware
   pio run --target upload
   ```
   O código-fonte (`src/main.cpp`) está descrito na seção [Firmware](#firmware-arduino--platformio) abaixo. Ele escuta a porta serial em `9600` baud, recebe comandos no formato `"anguloX,anguloY\n"` (ex.: `"95,88\n"`) e move os servos X e Y de acordo.
